...
const DEFAULT_TEAM: Team = {
  name: "DEFAULT TEAM",
  gladiators: [
    { token: 'BEAR', image: '/assets/team-selection/gladiators/bear-no-bg.png' },
    { token: 'MAGAIBA', image: '/assets/team-selection/gladiators/magaiba-no-bg.png' },
...
export default function BattlePreparation() {
  ...
  const [selectedTeam, setSelectedTeam] = useState<Team>(TEAMS[0] || DEFAULT_TEAM);
              ...
              <div className="grid grid-cols-3 gap-2 overflow-y-scroll overflow-x-hidden snap-y pr-4 pb-4 max-h-[234px]">
                {TEAMS.length > 0 ? (
                  TEAMS.map((team, index) => (
                    ...
                    >
                      <div className={`absolute inset-0 z-20 transition-shadow duration-300 ${
                        selectedTeam.name === team.name 
                          ? 'shadow-[inset_0_0_0_4px_#FFFF00]' 
                  ...
                  <div className="col-span-3 text-center">No teams available</div>
                )}
              </div>
            </div>
          </div>
            ...
            <div className="">
              ...
              <div className="flex gap-2 items-center justify-between">
                {Object.entries(STAKED_AMOUNTS).map(([token, amount]) => (
                  <div key={token} className="flex gap-2 items-center justify-between">
                    ...
                    <div key={index} className="bg-dark-blue-50 w-[113px]">
                      <div className="p-1">
                        ...
                        <div className="bg-dark-blue-70 mt-[-20px] w-full z-0 flex justify-center">
                          <p className="font-bold text-[10px]">{gladiator.token}</p>
                        </div>
                        <div className="py-1">
                        ...
                        <div className="flex items-center gap-1">
                          ...
                          <div>
                            <p className="text-light-blue font-medium text-[10px]">
                              {calculateTokenAmount(bets[index] || 0, TOKEN_PRICES[gladiator.token]).toLocaleString(undefined, {maximumFractionDigits: 2})}
                            </p>
                          </div>
                        </div>
                      </div>
